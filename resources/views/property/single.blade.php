    ...
    <div class="shadow-md border-2 border-gray-300 py-2 bg-white mt-28">
        <div class="container mx-auto">
            <ul class="flex items-center">
                <li><a class="text-red-800" href="{{ route('property.index') }}">{{ __('PROPERTI') }}</a></li>
                ...
                <div id="slider" class="">
                    <div class="gallery-slider">
                        @foreach($property->gallery as $gallery)
                            @if (file_exists(public_path('storage/app/uploads/'.$gallery->name)))
                    ...
                    <div class="thumbnail-slider mt-4">
                        @foreach($property->gallery as $gallery)
                            @if (file_exists(public_path('storage/uploads/'.$gallery->name)))
                                <div style="background-image: url('/storage/app/uploads/{{ $gallery->name }}')" class="single-thumbnail-item bg-cover bg-center"></div>
                            @else
